var resourceMappingConfig = require('../config/resourceMappingConfig_v15.0.json')['resource-mapping'];
var aclOperations = require('../support/aclOperations');

exports.up = function(db, next){
    console.log(resourceMappingConfig);
    if (resourceMappingConfig.length === 0) {
        console.log("Skipping resourceMappingConfig_v15.0.json as it there is no content");
        next();
    } else {
        ...
        aclOperationsPromise.then(function (results) {
            console.log("Loaded all ACL roles from resourceMappingConfig_v15.0.json");
            next();
        });
    }
};
